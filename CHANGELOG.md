...
- **`FetchHttpClient` GET/HEAD body leak**: The Fetch API throws a `TypeError` when a `GET` or `HEAD` request includes a body. `RequestBuilder` initialises `body` as `{}` by default, which is truthy, causing every `GET`/`HEAD` call to fail. `FetchHttpClient` now unconditionally omits the body for these methods.
...
- **`package.json` Metadata**: Added `repository`, `homepage`, and `bugs` fields so that the GitHub repository link, homepage, and issue tracker appear correctly on the npm package page.
